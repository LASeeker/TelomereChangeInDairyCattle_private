...
We used blood samples collected between 2008 and 2014 to measure relative leukocyte telomere length (RLTL) by monoplex qPCR in 1,325 samples from 305 individuals. 
...
On average 4.3 (range: 2-8) telomere measurements were made of each individual, including the first measurement within 15 days of birth and a variable number of subsequent measures (Figure S7 A). 

RLTL measures, adjusted for qPCR plate and row (see Methods), were approximately normally distributed (Figure S7 B). 
...
test_dat<-subset(data, data$sample_year != c(2008, 2009))
...
SubsPlot<-ggplot(redData, aes(x=as.numeric(paste(residual_rltl_tminus1)), y=redData$residual_rltl_t))+geom_point()+xlab("RLTL at time t-1")+ylab("RLTL at time t")+theme_classic(20)+geom_abline(intercept=0, slope=1, colour="red")+ stat_smooth(method=lm, formula=y~x)+
  annotate("text", x = -0.4, y = 0.5, label = "(B)", fontface =2, size = 12)


agePlot<- ggplot(redData, aes(x=as.factor(redData$age_y), y=as.numeric(paste(redData$diff_rltl_res))))+geom_jitter()+theme_classic(20)+ xlab("Age in years")+ ylab("RLTL change")+ geom_boxplot()+
  annotate("text", x = 1, y = 0.5, label = "(A)", fontface =2, size = 12)
...
spaghetti plot for animals with more than seven RLTL measurements for supplementary figure S4:
...
ggplot(uDeadData, aes(x = as.numeric(paste(uDeadData$herd_life)))) + 
	geom_histogram(colour= "black", fill = mycoloursP[4]) + xlab("Productive lifespan in days") +  
  geom_vline(xintercept= mean(as.numeric(paste(uDeadData$herd_life))), colour="red") + theme_bw(20) +
	stat_function(
		fun = function(x, mean, sd, n){
...
```{r}
produc <-as.numeric(uDeadData$amp_305_lactation)

mod2<-glm(as.numeric(paste(uDeadData$herd_life))~uDeadData$average_resid + produc)
summary(mod2)

AvPlot<-ggplot(uDeadData, aes(y=as.numeric(paste(uDeadData$herd_life)), x=average_resid))+geom_point()+  
  #stat_smooth(method=lm, formula = y~x,)+
  xlab("Mean RLTL")+ ylab("Productive lifespan")+theme_gray(20)
...
Testing the relationship between telomere change over life and productive lifespan:
...
mod3a<-glm(as.numeric(paste(uDeadData$herd_life)) ~ uDeadData$average_resid + as.numeric(paste(uDeadData$rltl_change))+ as.numeric(paste(uDeadData$abs_rltl_change))) 

summary(mod3a)
AIC(mod3a)

df3a<-as.data.frame(round(coef(summary(mod3a)),3))

#write.csv(df3a, "/Users/luiseseeker/Desktop/PNAS/df3a.csv")

# more than 2 samples

mod3b<-glm(as.numeric(paste(uDeadDataMore2$herd_life))~as.numeric(paste(uDeadDataMore2$rltl_change))+ as.numeric(paste(uDeadDataMore2$abs_rltl_change))+ uDeadDataMore2$average_resid)

summary(mod3b)
AIC(mod3b)

df3b<-as.data.frame(round(coef(summary(mod3b)),3))

#write.csv(df3b, "/Users/luiseseeker/Desktop/PNAS/df3b.csv")

mod3br<-glm(as.numeric(paste(uDeadDataMore2$herd_life))~as.numeric(paste(uDeadDataMore2$rltl_change)))

summary(mod3br)

df3br<-as.data.frame(round(coef(summary(mod3br)),3))

#write.csv(df3br, "/Users/luiseseeker/Desktop/PNAS/df3br.csv")

# with milk yield:
Mdata<-subset(uDeadData, uDeadData$amp_305_lactation!="NULL")
produc<-as.numeric(Mdata$amp_305_lactation)

mod3M<-glm(as.numeric(paste(Mdata$herd_life))~as.numeric(paste(Mdata$rltl_change)) + produc + as.numeric(paste(Mdata$abs_rltl_change))+ Mdata$average_resid)
...
df3M<-as.data.frame(round(coef(summary(mod3M)),3))

#write.csv(df3M, "/Users/luiseseeker/Desktop/PNAS/df3M.csv")

# backwards elimination
mod3Mr<-glm(as.numeric(paste(Mdata$herd_life))~as.numeric(paste(Mdata$rltl_change)) + produc)

summary(mod3Mr)
AIC(mod3Mr)


df3Mr<-as.data.frame(round(coef(summary(mod3Mr)),3))

#write.csv(df3Mr, "/Users/luiseseeker/Desktop/PNAS/df3Mr.csv")


#Prod and more than 2 samples:

M2data<-subset(uDeadDataMore2, uDeadDataMore2$amp_305_lactation!="NULL")
product<-as.numeric(M2data$amp_305_lactation)

mod3M2<-glm(as.numeric(paste(M2data$herd_life))~as.numeric(paste(M2data$rltl_change))+ as.numeric(paste(M2data$abs_rltl_change))+ M2data$average_resid + product)

summary(mod3M2)
AIC(mod3M2)

df3M2<-as.data.frame(round(coef(summary(mod3M2)),3))

#write.csv(df3M2, "/Users/luiseseeker/Desktop/PNAS/df3M2.csv")

#backwards elimination

mod3M2r<-glm(as.numeric(paste(M2data$herd_life))~as.numeric(paste(M2data$rltl_change))+ product)

summary(mod3M2r)
AIC(mod3M2r)

df3M2r<-as.data.frame(round(coef(summary(mod3M2r)),3))

#write.csv(df3M2r, "/Users/luiseseeker/Desktop/PNAS/df3M2r.csv")
...
#only samples that have been taken after 15 d of life
uDeadDataLP<-subset(uDeadDataL, uDeadDataL$amp_305_lactation!="NULL")
product<-as.numeric(uDeadDataLP$amp_305_lactation)

mod3L<-glm(as.numeric(paste(uDeadDataLP$herd_life))~as.numeric(paste(uDeadDataLP$rltl_change))+ product)
...
mod3a<-glm(as.numeric(paste(uDeadData$herd_life))~ as.numeric(paste(uDeadData$rltl_change))+ produc)

summary(mod3a)
AIC(mod3a)

mod3b<-glm(as.numeric(paste(uDeadData$herd_life))~ as.numeric(paste(uDeadData$abs_rltl_change))+produc)
summary(mod3b)           
AIC(mod3b)

AIC(mod3b)-AIC(mod3a)

#seperately mean RLTL change and mean absolute RLTL change are both significant. However, based on AIC values the model including mean RLTL change fits better. 

mod3c<-glm(as.numeric(paste(uDeadData$herd_life))~ as.numeric(paste(uDeadData$rltl_change))+ as.numeric(paste(uDeadData$abs_rltl_change))+ uDeadData$average_resid+produc)
             

summary(mod3c)

dfSum<-as.data.frame(round(coef(summary(mod3c)),3))

#write.csv(dfSum, "/Users/luiseseeker/Desktop/PNAS/S5_LifeSpan_RLTLchange.csv" )

mod3d<-glm(as.numeric(paste(uDeadData$herd_life))~ as.numeric(paste(uDeadData$rltl_change))+ as.numeric(paste(uDeadData$abs_rltl_change))+produc)
...
#changePlot<-ggplot(uDeadData, aes(y=as.numeric(paste(uDeadData$herd_life)), x=as.numeric(paste(uDeadData$rltl_change))))+
 # geom_point()+  
  #geom_smooth(method="lm", formula = y~x,)+
  #xlab("Mean RLTL change")+ 
  #ylab("Productive lifespan")+theme_gray(20)



#ABSchangePlot<-ggplot(uDeadData, aes(y=as.numeric(paste(uDeadData$herd_life)), x=as.numeric(paste(uDeadData$abs_rltl_change))))+geom_point()+  geom_smooth(method="lm", formula = y~x,)+xlab("Mean absolute RLTL change")+ ylab("Productive lifespan")+theme_gray(20)
...
ggplot(uDeadDataMore2, aes(y=as.numeric(paste(uDeadDataMore2$herd_life)), x=average_resid))+geom_point()+  
  #stat_smooth(method=lm, formula = y~x,)+
  xlab("Mean RLTL")+ ylab("Productive lifespan")+theme_gray(20)
...
More2Plot<-ggplot(uDeadDataMore2, aes(y=as.numeric(paste(uDeadDataMore2$herd_life)), x=as.numeric(paste(uDeadDataMore2$rltl_change))))+geom_point()+ geom_smooth(method="lm", formula = y~x, se=TRUE)+xlab("Mean RLTL change (reduced)")+ ylab("Productive lifespan")+theme_gray(20)

#More2Plot

 
#grid.arrange(AvPlot, changePlot, More2Plot, ABSchangePlot, ncol=4)

#g<-arrangeGrob(AvPlot, changePlot, More2Plot, ABSchangePlot, ncol=4)

#ggsave(file="/Users/luiseseeker/Desktop/PNAS/ChangeLife.pdf", g, width = 20, height = 5)
```

Plot relationship between RLTL change and productive lifespan pre-adjusted for milk yield

```{r}
lm1<-lm(as.numeric(paste(Mdata$herd_life))~as.numeric(paste(Mdata$amp_305_lactation)))
summary(lm1)

Mdata$MilkAdjustedHerdLife<-resid(lm1)

lm2<-lm(Mdata$MilkAdjustedHerdLife~Mdata$rltl_change)
summary(lm2)

AdjHLplot<-ggplot(Mdata, aes(y=as.numeric(paste(Mdata$MilkAdjustedHerdLife)), x=as.numeric(paste(Mdata$rltl_change))))+geom_point()+ geom_smooth(method="lm", formula = y~x, se=TRUE)+xlab("Mean RLTL change")+ ylab("Adjusted prod. lifespan")+theme_gray(20)

AdjHLplot
```


## Analysis of change between first two measurements within first year and produrctive lifespan:
...
# for the linear model, I have to exclude animals with no clear phenotype, which means animals that were still alive at the 1.1.2017 are excluded
InData2Dead<-subset(InData2, InData2$herd_life!="NULL")

mod4<-glm(as.numeric(paste(InData2Dead$herd_life))~ InData2Dead$ChangeRate)
...
inDf<-as.data.frame(round(coef(summary(mod4)),3))
#write.csv(inDf, "/Users/luiseseeker/Desktop/PNAS/InitialChange.csv")


mod4b<-glm(as.numeric(paste(InData2Dead$herd_life))~ InData2Dead$ResDiff)
...
```{r}
inDfb<-as.data.frame(round(coef(summary(mod4b)),3))
#write.csv(inDfb, "/Users/luiseseeker/Desktop/PNAS/InitialChange_b.csv")


#For a cox proportional harzard analysis, I don't have to exclude animals that are still alive which may increase the statistical power. 



#early life measurements (at 0 years of age and 1 year of age) 
InChange<-as.numeric(paste(InData2$ResDiff))
hist(InChange)
coxFit<-coxph(Surv(InData2$TimeMeasure, InData2$Event == 1)~InData2$ResDiff)
...
```{r}
ggplot(InData2, aes(x=InData2$ResDiff)) +geom_histogram(colour= "black", fill = mycoloursP[4])+ xlab("Initial RLTL change") + theme_classic(20)
...
# change on discrete change to plot
coxFit<-coxph(Surv(InData2$TimeMeasure, InData2$Event == 1)~InData2$ResDiff_3G)

summary(coxFit)

coxFit<-survfit(Surv(InData2$TimeMeasure, InData2$Event == 1)~InData2$ResDiff_3G)
summary(coxFit)
...
write.csv(InData2, "/Users/luise/Documents/Work/CattleTelomereLength/AgingCell/rawdata/CalfSurv.csv")
#### plot results for discrete scale

#ggsurvplot gives p-values that are different to the ones above calculated with coxph. I don't know where they are taken from therefor decided to report the ones I understand, the ones calculated with coxph:

g1<- ggsurvplot(coxFit, data=InData2, pval = 0.006, ggtheme = theme_minimal(), risk.table.y.text.col = T, risk.table.y.text = FALSE, legend.labs=c("Tertile 1 (most RLTL attrition)", "Tertile 2", "Tertile 3 (least RLTL attrition"))+ guides(colour = guide_legend(nrow = 3))+theme_survminer(40)

  
ggsurvplot(coxFit, data=InData2, pval = 0.006, ggtheme = theme_minimal(), risk.table.y.text.col = T, risk.table.y.text = FALSE, legend.labs=c("Tertile 1 (most RLTL attrition)", "Tertile 2", "Tertile 3 (least RLTL attrition"))+theme_survminer(40)


ggsave(file = "/Users/luise/Documents/Work/CattleTelomereLength/AgingCell/Images/InitialChange.pdf", print(g1))


#in response to editor plot as dotplot
dInData<-subset(InData2, InData2$cull_date != "NULL")

ggplot(dInData, aes(x=as.numeric(paste(dInData$herd_life)), y= ResDiff)) + geom_point() + xlab("productive life span")+ylab("RLTL change")+ geom_smooth(method="lm", formula = y~x, se=TRUE)+ theme_grey(20)
```


```{r}
ggplot(InData2, aes(x=as.factor(InData2$ResDiff_3G), y=InData2$ResDiff)) + geom_boxplot() +xlab("RLTL change tertiles") + ylab("RLTL change")+ theme_light(20)+ geom_jitter()
...
g2<-ggsurvplot(coxFit, data=coxData, pval = 0.858, ggtheme = theme_minimal(), risk.table.y.text.col = T, risk.table = FALSE, legend.labs=c("Tertile 1 (shortest mean RLTL)", "Tertile 2", "Tertile 3 (longest mean RLTL)"))+theme_survminer(40)+ guides(colour = guide_legend(nrow = 3))

ggsurvplot(coxFit, data=coxData, pval = 0.858, risk.table.y.text.col = T, risk.table = TRUE, legend.labs=c("Tertile 1 (shortest mean RLTL)", "Tertile 2", "Tertile 3 (longest mean RLTL)"))+ guides(colour = guide_legend(nrow = 3))+ theme_survminer(10)


#ggsurvplot gives p-values that are different to the ones above calculated with coxph. I don't know where they are taken from therefor decided to report the ones I understand, the ones calculated with coxph:

ggsurvplot(coxFit, data=coxData, pval = 0.858, ggtheme = theme_minimal(), risk.table.y.text.col = T, risk.table.y.text = TRUE, legend.labs=c("Tertile 1 (shortest mean RLTL)", "Tertile 2", "Tertile 3 (longest mean RLTL)"))+theme_survminer(40)
...
#in response to editor plot as dotplot
dData<-subset(coxData, coxData$cull_date != "NULL")

ggplot(dData, aes(x=as.numeric(paste(dData$herd_life)), y= rltl_change)) + geom_point() + xlab("productive life span")+ylab("Mean RLTL change")+ geom_smooth(method="lm", formula = y~x, se=TRUE)+ theme_grey(20)

ggplot(dData, aes(x=as.numeric(paste(dData$herd_life)), y= rltl_change)) + geom_point() + xlab("productive life span")+ylab("Mean RLTL change")+ theme_grey(20)

ggplot(dData, aes(x=as.numeric(paste(dData$herd_life)), y= abs_rltl_change)) + geom_point() + xlab("productive life span")+ylab("Mean absolute RLTL change")+ theme_grey(20) +geom_smooth(method="lm", formula = y~x, se=TRUE)

ggplot(dData, aes(x=as.numeric(paste(dData$herd_life)), y= average_resid)) + geom_point() + xlab("productive life span")+ylab("Mean RLTL")+ theme_grey(20) +geom_smooth(method="lm", formula = y~x, se=TRUE)
...
```{r}
## continuous scale
coxFit<-coxph(Surv(coxData$TimeMeasure, coxData$Event == 1)~coxData$abs_rltl_change)

summary(coxFit)

### discrete scale
coxFit<-coxph(Surv(coxData$TimeMeasure, coxData$Event == 1)~coxData$abs_rltl_change_4G)

summary(coxFit)


coxFit<-survfit(Surv(coxData$TimeMeasure, coxData$Event == 1)~coxData$abs_rltl_change_4G)


#### plot results for discrete scale

ChangeQuart<-ggsurvplot(coxFit, data=coxData, pval = TRUE, ggtheme = theme_minimal(), risk.table.y.text.col = T, risk.table.y.text = FALSE, legend.labs=c("Quartile 1 (least RLTL change)", "Quartile 2", "Quartile 3", "Quartile 4 (most RLTL change)"))+theme_survminer(40)+ guides(colour = guide_legend(nrow = 3))

ggsave(file = "/Users/luise/Documents/Work/CattleTelomereLength/AgingCell/Images/ChangeQuartRLTL.pdf", print(ChangeQuart))
...
### discrete scale
coxFit<-survfit(Surv(coxData$TimeMeasure, coxData$Event == 1)~coxData$rltl_change_4G)

summary(coxFit)

#### plot results for discrete scale

attr<-ggsurvplot(coxFit, data=coxData, pval = TRUE, ggtheme = theme_minimal(), risk.table.y.text.col = T, risk.table.y.text = FALSE, legend.labs=c("Quartile 1 (most RLTL attrition)", "Quartile 2", "Quartile 3", "Quartile 4 (least RLTL attrition)")) +theme_survminer(40)

ggsave(file = "/Users/luise/Documents/Work/CattleTelomereLength/AgingCell/Images/RLTLattrition.pdf", print(attr))
...
coxFit<-coxph(Surv(coxData$TimeMeasure, coxData$Event == 1)~coxData$RLTL_1)
...
#now I will include average milk production
...
coxFit<-coxph(Surv(coxDataR$TimeMeasure, coxDataR$Event == 1)~coxDataR$RLTL_1 + as.numeric(paste(coxDataR$amp_305_lactation))+ coxDataR$totalTELnumber)
...
#try RLTL at the age of 1 year in the same model

coxFit<-coxph(Surv(coxDataR$TimeMeasure, coxDataR$Event == 1)~coxDataR$rltl_change+ coxDataR$RLTL_1+ as.numeric(paste(coxDataR$amp_305_lactation)))

summary(coxFit)



coxFit<-coxph(Surv(coxDataR$TimeMeasure, coxDataR$Event == 1)~coxDataR$RLTL_1 + as.numeric(paste(coxDataR$amp_305_lactation)))

summary(coxFit)

#all three measures in the same model:
...
#Now I try the same without the fist measurement. 
uRedDataL<- redDataLnew[!duplicated(redDataLnew$recoded_id),]

coxDataL<-data.frame(recoded_id=uRedDataL$recoded_id, dob=uRedDataL$dob, herd_life=uRedDataL$herd_life, cull_date=uRedDataL$cull_date, average_resid=uRedDataL$average_resid, rltl_change=uRedDataL$rltl_change, abs_rltl_change=uRedDataL$abs_rltl_change, amp_305_lactation= uRedDataL$amp_305_lactation)
...
coxData2<-data.frame(recoded_id=uDeadDataMore2$recoded_id, dob=uDeadDataMore2$dob, herd_life=uDeadDataMore2$herd_life, cull_date=uDeadDataMore2$cull_date, average_resid=uDeadDataMore2$average_resid, rltl_change=uDeadDataMore2$rltl_change, abs_rltl_change=uDeadDataMore2$abs_rltl_change, amp_305_lactation=uDeadDataMore2$amp_305_lactation, RLTL_1=uDeadDataMore2$RLTL_1)


#add maximum sample number 

coxData2<-merge(coxData2, maxSN, by = "recoded_id")
...
#add groups for all lifelong change measurements that are split at the median
coxData2$AverageResi_2G<-ifelse(coxData2$average_resid <= median(coxData2$average_resid), 1, 2)
coxData2$rltl_change_2G<-ifelse(coxData2$rltl_change <= median (coxData2$rltl_change), 1, 2 )
coxData2$abs_rltl_change_2G<-ifelse(coxData2$abs_rltl_change <= median (coxData2$abs_rltl_change), 1, 2 )

#split at quartiles


coxData2$average_resid_3G<-ntile(coxData2$average_resid,3)
coxData2$rltl_change_3G<-ntile(coxData2$rltl_change,3)
coxData2$abs_rltl_change_3G<-ntile(coxData2$abs_rltl_change,3)

# models:
##Average lifetime telomere length
### condinuous variable:
coxFit<-coxph(Surv(coxData2$TimeMeasure, coxData2$Event == 1)~coxData2$rltl_change)
...
ggplot(data, aes(x = variable)) + 
	geom_histogram(colour= "black", fill = "yellow") + xlab("Sample interval in days") +   geom_vline(xintercept= meanVar,  colour="red")+
  theme_gray(20)

  #+
	#stat_function(
	#	fun = function(x, mean, sd, n){
	#		 n*50* dnorm(x = x, mean = mean, sd = sd)
	#	}, 
	#	args = with(data, c(mean = meanVar, sd = sdVar, n = length(variable))))
...
ggplot(uData, aes(x = variable)) + 
	geom_bar(colour= "black", fill = "yellow") + xlab("Birth year")+ theme_gray(20)
...
ggplot(uData, aes(x = variable)) + 
	geom_histogram(colour= "black", fill = "yellow") + xlab("Average milk production") +   geom_vline(xintercept= meanVar,  colour="red")+
  theme_gray(20)

# cull reasons

variable<-as.factor(paste(uDeadData$cull_reson))

ggplot(uDeadData, aes(x = variable)) + 
	geom_bar(colour= "black", fill = "yellow") + xlab("Cull reason")+
...
ggplot(newdata, aes(x = variable)) + 
	geom_bar(colour= "black", fill = "yellow") + xlab("Fertility events")+
  theme_gray(20)+
...
ggplot(newdata, aes(x = variable)) + 
	geom_bar(colour= "black", fill = "yellow") + xlab("Mastitis events")+
  theme_gray(20)+
...
ggplot(newdata, aes(x = variable)) + 
	geom_bar(colour= "black", fill = "yellow") + xlab("Lameness events")+
  theme_gray(20)+
